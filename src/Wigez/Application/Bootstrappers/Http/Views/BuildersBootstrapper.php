...
namespace Wigez\Application\Bootstrappers\Http\Views;
...
use Wigez\Application\Http\Views\Builders\AdminBuilder;
use Wigez\Application\Http\Views\Builders\HtmlErrorBuilder;
use Wigez\Application\Http\Views\Builders\WebsiteBuilder;
...
class BuildersBootstrapper extends Bootstrapper
{
    ...
    public function run(IViewFactory $viewFactory)
    {
        $viewFactory->registerBuilder(
            'layouts/website', function (IView $view) {
            /** @see WebsiteBuilder::build() */
            return (new WebsiteBuilder())->build($view);
        }
        );
        $viewFactory->registerBuilder(
            'layouts/admin', function (IView $view) {
        ...
        $viewFactory->registerBuilder(
            'layouts/empty', function (IView $view) {
            /** @see AdminBuilder::build() */
            return (new WebsiteBuilder())->build($view);
        }
        );
        $viewFactory->registerBuilder(
            'errors/html/Error', function (IView $view) {
